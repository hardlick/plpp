...
class plexAPI {
    ...
    public function setExcludedLibraries($excludedLibraries) {
        if (is_array($excludedLibraries)) {
            $this->plexExcludedLibraries = $excludedLibraries;
        }
    }
    ...
    function getFormatedItemsContentBy($itemsKey, $contentType, $content, $format, $item) {
        $content = $this->getItemsContent($itemsKey, $contentType, $content, $item);
        switch ($format) {
            case 'img': {
                    if (empty($this->plexItems[$item]['items'][$itemsKey]['thumb']) || ($this->plexItems[$item]['items'][$itemsKey]['thumb'] == $this->plexItems[$item]['items'][$itemsKey]['parentThumb'])) {
    ...
    public function isSetContent($itemsKey, $contentType, $content, $item) {
        ...
        switch ($contentType) {
            case 'attribute': {
                    if (isset($this->plexItems[$item][$content])) {
                        $return = true;
                    }
                    break;
                }
            case 'iteminfo': {
                    if (isset($this->plexItems[$item]['items'][$itemsKey][$content])) {
                        $return = true;
                    }
                    break;
                }
            case 'mediainfo': {
                    if (isset($this->plexItems[$item]['items'][$itemsKey]['Media'][$content])) {
                        $return = true;
                    }
                    break;
                }
    ...
    private function getXMLN($path = '', $query = array()) {
        $query['X-Plex-Token'] = $this->plexServer['token'];

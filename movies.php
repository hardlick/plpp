...
if ($plppViewmode == 'img') {
    ...
    // In all other cases, we load the image from the plex server and process it
        ...
        if ($plppItemsFilter != 'full') {
            ...
            imagecopyresampled($plppThumb, $plppImage, 0 - ($plppThumbNewlWidth - $plppImageWidth) / 2, // Center the image horizontally
                    0 - ($plppThumbNewlHeight - $plppImageHeight) / 2, // Center the image vertically
                    0, 0, $plppThumbNewlWidth, $plppThumbNewlHeight, $plppImageOriginalWidth, $plppImageOriginalHeight);
...
if ($plppConfiguration['usersettings']['search_link']) {
    $plppOutput['Menu'] .= '<li class="plpp_menu"><a href="#" data-target="#myPlexModalSearch"><span class="plpp_menu plpp_menu_search">Buscar...</span></a></li>' . PHP_EOL;
}
// Add link for Settings
if ($plppConfiguration['usersettings']['settings_link']) {
    $plppOutput['Menu'] .= '<li class="plpp_menu"><a href="settings.php"><span class="plpp_menu plpp_menu_settings">Configuracion</span></a></li>' . PHP_EOL;
}
// Add link for debug data
if ($plppConfiguration['usersettings']['debug']) {
    $plppOutput['Menu'] .= '<li class="plpp_menu"><a href="#" data-target="#myPlexModalDebug"><span class="plpp_menu plpp_menu_debug">Mostrar info Debug</span></a></li>' . PHP_EOL;
}
...
foreach ($plppItems as $parentKey => $parent) {
    ...
    if (!empty($parent['title2'])) {
        if ($parent['title2'] == 'All Películas') {
            $parent['title2'] = 'Todas las Películas';
        } elseif ($parent['title2'] == 'Recently Added') {
            $parent['title2'] = 'Agregados Recientemente';
        } elseif ($parent['title2'] == 'Recently Released') {
        ...
        if ($plppItemType == 'library' && $plppItem != '') {
            ...
            foreach ($filters as $key => $value) {
                if ($value == 'All Películas') {
                    $value = 'Todas las Películas';
                } elseif ($value == 'Recently Added') {
                    $value = 'Agregados Recientemente';
                } elseif ($value == 'Recently Released') {
                    $value = 'Estrenados Recientemente';
                }
